...
export class EquityService extends BaseBankService {
  async initiatePayment(request: BankPaymentRequest): Promise<BankPaymentResponse> {
    try {
      ...
      // Equity Bank API specific implementation
      const response = await axios.post(
        `${this.config.api_endpoint}/transactions/initiate`,
        {
          amount: request.amount,
          merchant_reference: reference,
          customer_name: request.student_name,
          customer_id: request.student_admission_number,
      ...
      console.error('Equity payment initiation failed:', error);
  ...
  async verifyPayment(transactionId: string): Promise<BankPaymentResponse> {
    try {
      const response = await axios.get(
        `${this.config.api_endpoint}/transactions/status/${transactionId}`,
        {
          headers: {
            'Authorization': `Bearer ${this.config.api_key}`,
            'X-API-Key': this.config.api_key
      ...
      console.error('Equity payment verification failed:', error);
      return {
        success: false,
        transaction_id: transactionId,
        reference: '',
  ...
  async validateWebhook(data: unknown): Promise<BankWebhookData> {
    try {
      ...
      return {
        ...
        status: this.mapEquityStatus(webhookData.status),
        timestamp: webhookData.timestamp,
        additional_data: webhookData.additional_data
      };
    } catch (error) {
      console.error('Equity webhook validation failed:', error);
      throw error;
    }
  }

  private verifySignature(signature: string, data: unknown): boolean {
    // Implement Equity-specific signature verification
    // This is a placeholder - implement according to Equity's documentation
    return true;
  }

  private mapEquityStatus(status: string): 'success' | 'failed' | 'pending' {
